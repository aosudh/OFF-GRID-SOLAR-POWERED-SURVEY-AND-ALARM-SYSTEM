...
void lcdBacklight(){
  ...
  if(backlightSleepMode==0){prevLCDBackLMillis = millis();}                 //变量为永不休眠
  else if(backlightSleepMode==1){backLightInterval=10000;}                  //背光 var 在 10 秒后休眠，无需按键
  else if(backlightSleepMode==2){backLightInterval=300000;}                 //设置为 5 分钟后休眠
  else if(backlightSleepMode==3){backLightInterval=3600000;}                //设置为 1 小时后休眠  
  else if(backlightSleepMode==4){backLightInterval=21600000;}               //设置为 6 小时后休眠   
  else if(backlightSleepMode==5){backLightInterval=43200000;}               //设置为 12 小时后休眠   
  else if(backlightSleepMode==6){backLightInterval=86400000;}               //设置为 1 天后休眠 
  else if(backlightSleepMode==7){backLightInterval=259200000;}              //设置为 3 天后休眠 
  else if(backlightSleepMode==8){backLightInterval=604800000;}              //设置为 7 天后休眠   
  else if(backlightSleepMode==9){backLightInterval=2419200000;}             //设置为 1 月后休眠    

  if(backlightSleepMode>0 && settingMode==0){
    currentLCDBackLMillis = millis();
    if(currentLCDBackLMillis-prevLCDBackLMillis>=backLightInterval){        // 运行例程
      prevLCDBackLMillis = currentLCDBackLMillis;                           //设置背光
      lcd.setBacklight(LOW);                                                //递增时间计数器
    } 
  }  
}
